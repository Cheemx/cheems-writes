...
func GetAllTechBlogs(c *gin.Context) {
	collection := config.GetCollection("techblogs")
...
func GetTechBlogBySlug(c *gin.Context) {
	slug := c.Param("slug")
	collection := config.GetCollection("techblogs")
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
...
func UpdateTechBlog(c *gin.Context) {
	slug := c.Param("slug")
	var updatedData models.TechBlog
...
func DeleteTechBlog(c *gin.Context) {
	slug := c.Param("slug")
	collection := config.GetCollection("techblogs")
...
func CreateTechBlog(c *gin.Context) {
	var blog models.TechBlog
